...
import { useCreateIncome, useIncomeList } from '../hooks/useFinance';
...
export const Income: React.FC = () => {
  ...
  const handleCreate = (payload: IncomePayload) => createMutation.mutateAsync(payload);
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
          ...
          <FormControl size="small" sx={{ minWidth: 180 }}>
            ...
            <Select
              ...
              label="Category"
              onChange={(event) => setCategoryFilter(event.target.value as IncomeCategory | '')}
            >
          ...
          <Table size="medium">
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} align="center" sx={{ py: 6 }}>
                    <Typography sx={{ color: '#64748B' }}>No income records found.</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                data?.results.map((income) => (
                  <TableRow key={income.id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    <TableCell>
                      <Typography variant="body2">{formatDate(income.income_date)}</Typography>
                    </TableCell>
                    <TableCell>
                      <Chip
                        label={income.category_display}
                        size="small"
                    ...
                    <TableCell>
                      <Typography variant="body2">{income.reference || '-'}</Typography>
                    </TableCell>
                    <TableCell>
